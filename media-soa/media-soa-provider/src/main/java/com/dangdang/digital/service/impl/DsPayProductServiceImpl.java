...
public class DsPayProductServiceImpl extends BaseServiceImpl<DsPayProduct, Integer> implements IDsPayProductService {
	...
	public List<String> getDSPayRelationProductIdByMoney(Integer money)
		...
		List<DsPayProduct> dsPayProductList = this.dao.selectList("DsPayProductMapper.selectByMoneys", param);
		...
		for(DsPayProduct dsPayProduct : dsPayProductList){
			if(dsPayProduct.getMoney() != null && !tempResult.containsKey(dsPayProduct.getMoney())){
				tempResult.put(dsPayProduct.getMoney(), dsPayProduct.getProductId());
	...
	public List<String> getDepositRelationProduct(List<String> productIds) {
		...
		return (List<String>)this.dao.getSqlSessionQueryTemplate().selectList("DsPayProductMapper.getDepositRelationProduct", map);
	}
