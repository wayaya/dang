...
public class ColumnContentServiceImpl extends BaseServiceImpl<ColumnContent, Integer> implements IColumnContentService {
	...
	public void updateBatch(List<ColumnContent> contents) {
		...
		//dao.updateBatch(contents);
	}
	
	public List<Long> getSaleIdsByColumnCode(int offset,int count,String columnCode){
		return dao.getSaleIdsByColumnCode(columnCode);
	}
	
	@Override
	public List<ColumnContent> getColumnContentBySaleIds(List<Long> saleIdList) {
		return dao.selectList("ColumnContentMapper.getColumnContentBySaleIds", saleIdList);
	}
}
