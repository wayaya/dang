	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$('.pagination').pagination(${pageFinder.rowCount?c}, {
				...
				link_to: encodeURI('/digest/findDigestByAuthorId.go?page=__id__'+condition)
		    });
		...
		function makecondition(){
			...
			if(authorId.length  > 0){
				condition = condition + "&authorId="+authorId;
			}
  ...
  <body>
	 <div class="page"><!--page开始-->
		...
		<div class="main clear"><!--main开始-->
			...
			<div class="right">
				<div class="m-r">
					<h3>******当心好文管理&gt;&gt;作者文章列表页</h3>
				    <div>	
				    <table class="table2">
			      		 <input type="hidden" name="authorId" id="authorId" value="<#if authorId??>${authorId?c}</#if>">
		            	<tr>
  ...
  <script>
  	$(document).ready(function(){
		$('#chkAll').click(function(){
			if(this.checked){
				 $('[name=disId]:checkbox').attr("checked", true);
			}else{
				 $('[name=disId]:checkbox').attr("checked", false);
			}
		});
});
  </script>
