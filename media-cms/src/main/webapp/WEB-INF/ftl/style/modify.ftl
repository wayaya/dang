		...
		<div class="main clear"><!--main开始-->
			<div class="right">
				<div class="m-r">
				    ...
				    <div>
					    <form action="/style/update.go" method="post" id="activity_type_add_form">
					    <input type="hidden" name="styleId" value="<#if style??><#if style.styleId??>${style.styleId?c}</#if></#if>">
						     <table class="table3" border="1" bordercolor="#a0c6e5" rules=none>
			    				<tr>
					    			<td class="tdright">名称：</td><td><input type="text" name="name" id="name" value="<#if style??><#if style.name??>${style.name}</#if></#if>" onblur="validInput('name')"></td><td style="width:1100px" id="nameInfo"></td>
						      	</tr>
						      	<tr>
					    			<td>编号：</td><td><input type="text" value="<#if style??><#if style.code??>${style.code}</#if></#if>" name="code" id="code" onblur="validInput('code')"></td><td style="width:1100px" id="codeInfo"></td>
						      	</tr>
						      	
						      	<tr>
					    			<td>是否有效：</td><td>
					    				<select id="status" name="status" onblur="validInput('status')">
					    					<option value="1" <#if style??><#if style.status??><#if style.status==1>selected="selected"</#if></#if></#if>>有效</option>
					    					<option value="0" <#if style??><#if style.status??><#if style.status==0>selected="selected"</#if></#if></#if>>无效</option>
					    				</select>
					    			</td>
					    			<td style="width:1100px" id="statusInfo"></td>
						      	</tr>
						      	<tr>
					    			<td>描述：</td><td><input type="text" value="<#if style??><#if style.desc??>${style.desc}</#if></#if>" name="desc" id="desc"  onblur="validInput('desc')"></td><td style="width:1100px" id="descInfo"></td>
						      	</tr>
						      	<tr>
