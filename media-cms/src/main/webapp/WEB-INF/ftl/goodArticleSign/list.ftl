...
div#rMenu ul li{
  ...
  <TABLE border=0 height=600px align=left>
	<TR>
		...
		<IFRAME ID="testIframe" Name="testIframe" FRAMEBORDER=0 SCROLLING=AUTO width=100%  height=100% SRC="/goodArticleSign/list.go">
		</IFRAME></TD>
	</TR>
</TABLE>
</div>
...
<div id="rMenu">
		...
		callback: {
			beforeClick: function(treeId, treeNode) {
				...
				if (treeNode.isParent && treeNode.id !=0) {
					demoIframe.attr("src","/goodArticleSign/list.go?parentId="+treeNode.id);
					zTree.expandNode(treeNode);
					return false;
				} else {
					//demoIframe.attr("src",treeNode.file + ".html");
					demoIframe.attr("src","/goodArticleSign/list.go?id="+treeNode.id);
					return true;
				}
			},
	...
	function addNode(){
		...
		if(nodes && nodes.length>0){
			demoIframe.attr("src","/goodArticleSign/toAdd.go?parentId="+nodes[0].id);
		}
		hideRMenu();
	}
	function checkTreeNode(){
		var nodes = zTree.getSelectedNodes();
		if(nodes && nodes.length>0){
			demoIframe.attr("src","/goodArticleSign/toEdit.go?id="+nodes[0].id);
		}
		hideRMenu();
	}
	function removeTreeNode() {
			...
			if (nodes && nodes.length>0) {
				if (nodes[0].children && nodes[0].children.length > 0) {
					...
					location = "/goodArticleSign/del.go?id="+nodes[0].id;
				}
			}
		}
	function hideRMenu() {
