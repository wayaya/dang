...
article, aside, figure, footer, header, hgroup, 
  ...
  <body>
	 <div class="page"><!--page开始-->
		<div class="main clear"><!--main开始-->
			...
			<td style="width:82%;height:100%;">
				<iframe width="100%" height="100%" src="/goodArticleSign/list.go"></iframe>
			    </td>
			    </tr>
			    </table>
		    </div>
    ...
    <div name="add" class="add">添加子标签</div>  
...
function save(){
	...
	var node;
	$.post("/goodArticleSign/add.go", params,
			   function(data){
			     if(data.success){
			    	 $('#win').window('close');
			    	 $.post("/goodArticleSign/tree.go", 
			  			   function(data){
			    		 treeDate(data);
			  			   });
			     }else{
...
$(function () {
    ...
    function click(menu, type) {
        ...
        if(type == 'del'){
        	$.post("/goodArticleSign/del.go?id="+node.id, 
     			   function(data){
        		if(data.success){
			    	 $.post("/goodArticleSign/tree.go", 
			  			   function(data){
