	 ...
	 <div class="page"><!--page开始-->
		<div class="main clear"><!--main开始-->
			<div class="right" style="width:99%;">
				<div class="m-r">
					<h3>专题管里&gt;&gt;修改专题&nbsp;&nbsp;<image src="/images/return.jpg" onclick="javascript:history.go(-1)" /></h3>
					<#if returnInfo??>
						<div class="mrdiv">
				      		 <table >
			        			<tr>
				    ...
				    <div>
					    <form action="/block/special/update.go" method="post" id="add_form" enctype="multipart/form-data">
					    <input type="hidden" name="stId" id="stId" value="<#if specialTopic??><#if specialTopic.stId??>${specialTopic.stId?c}</#if></#if>">
					    <input type="hidden" id="stTypeId" name="stTypeId" title="专题所属类型编号" value="${specialTopic.stTypeId?c}">
					    <input type="hidden" id="columnCode" name="columnCode" value="<#if specialTopic.column??>${specialTopic.column.columnCode}</#if>">
						     <table class="table2" border="1" bordercolor="#a0c6e5" rules="none">
					     	    <tr>
						      	...
						      	<tr>
					    			...
					    			<td class="tdleft">
					    				<select id="type" name="deviceType" onblur="validInput('deviceType')">
					    					<option value="YC_All" <#if specialTopic??><#if specialTopic.deviceType??><#if specialTopic.deviceType=='YC_All'>selected="selected"</#if></#if></#if>>原创通用</option>
						      	...
						      	<tr id="zhuanti_5" style="height: 80px;">
					    			<td class="tdright">（新）主图：</td>
					    			<td  style="text-align:left">
					    				<img id="preview" alt="新主图预览" name="pic"  />
					    			</td>
						      	</tr>
						      	<#if specialTopic??>
						      	...
						      	<td class="tdleft">
									<br><input type="text" id="columnName" name="columnName"  value="<#if specialTopic.column ??>${specialTopic.column.name}</#if>" readonly="true" >
									<button type="button" id="column" name="column">关联栏目</button>
									<span id="idInfo"></span>
									</td>
								</tr>
								<tr>
					    			<td class="tdright">排序值：</td><td class="tdleft">
					    			<input type="text" name="sort" id="sort" value="<#if specialTopic.sort ??>${specialTopic.sort}</#if>">
					    			<span style="width:1100px;text-align:left" id="sortInfo"  name="sortInfo">该数字必须为正整数，且该值越大专题月靠前</span>
					    			</td>
						      	</tr>
								<tr>
					    			<td class="tdright">是否首页显示：</td>
					    			<td class="tdleft" style="text-align: left;">
					    			<input type="radio" id="showHomePage" name="showHomePage" value="1" <#if specialTopic??><#if specialTopic.showHomePage??><#if specialTopic.showHomePage==1>checked="true"</#if></#if></#if>>是</input>
					    			<input type="radio" id="showHomePage" name="showHomePage" value="0" <#if specialTopic??><#if specialTopic.showHomePage??><#if specialTopic.showHomePage==0>checked="true"</#if><#else>checked="true"</#if></#if>>否</input>
									</td>
						      	</tr>
						      	<tr>
					    			<td class="tdright">状态：</td>
					    			<td class="tdleft" style="text-align: left;">
					    			<input type="radio" id="status" name="status" value="1" <#if specialTopic??><#if specialTopic.status??><#if specialTopic.status==1>checked="true"</#if></#if></#if> >有效</input>
					    			<input type="radio" id="status" name="status" value="0" <#if specialTopic??><#if specialTopic.status??><#if specialTopic.status==0>checked="true"</#if></#if></#if>>无效</input>
									</td>
						      	</tr>
						      	<tr>
						      	<td class="tdright">说明:</td>
						      	<td class="tdleft">
						      	<textarea id="description" name="description" rows="3" cols="60"><#if specialTopic.description??>${specialTopic.description}</#if></textarea>
						      	</td>
						      	</tr>
						      	<#if returnInfo??>
							    <#else>
