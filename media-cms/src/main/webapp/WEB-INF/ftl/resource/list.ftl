  ...
  <TABLE border=0 height=600px align=left>
	<TR>
		...
		<TD width="88%" align=left valign=top>
			<IFRAME ID="testIframe" Name="testIframe" FRAMEBORDER=0 width="100%"  height="990px" SRC="/mediaResource/list.go?isCdn=${isCdn}&parentId="+<#if cateId??>${cateId}</#if>>
		</IFRAME></TD>
	</TR>
</TABLE>
</div>
...
<div id="rootMenu" style="display:none;">
		...
		callback: {
			beforeClick: function(treeId, treeNode) {
				...
				if (treeNode.isParent && treeNode.id !=0) {
					demoIframe.attr("src","/mediaResource/list.go?dirId="+treeNode.id+"&isCdn="+${isCdn});
	...
	function addNode(){
		...
		if(nodes && nodes.length>0){
			demoIframe.attr("src","/mediaResource/toAdd.go?parentId="+nodes[0].id+"&isCdn="+${isCdn});
		}
		hideRMenu();
	}
	...
	function removeTreeNode() {
			...
			if (nodes && nodes.length>0) {
				if (window.confirm('你确定要删除吗？')) {
					$.post("/mediaResource/delDir.go", {dirId:nodes[0].id},
			   			function(data){
			     			if(data.success){
			    				location = "/mediaResource/main.go?isCdn=${isCdn}";
			     			}else{
