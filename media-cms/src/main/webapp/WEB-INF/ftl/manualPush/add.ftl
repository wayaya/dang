...
<form id="add" name="add" method="post" action="/manualPush/addSubmit.go" >
	...
	<input type="hidden" name="planStatus" value="0" />
	...
	<table class="table2">
		...
		<#if userSessionInfo?? && userSessionInfo.f['182']?? >
			<td colspan="2"><button type="button" onclick="checkForm()">确认</button>
		</#if>
		&nbsp;&nbsp;<button type="button"  onclick="javascript:window.location.href='/manualPush/list.go'">取消</button></td>
		</tr>
	</table>
</form>
</div>
