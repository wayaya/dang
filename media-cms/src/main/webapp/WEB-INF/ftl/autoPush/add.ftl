...
<form id="add" name="add" method="post" action="/autoPush/addSubmit.go" >
	<input type="hidden" name="planType" value="2" />
	<input type="hidden" name="pushedNumber" value="0" />
	<input type="hidden" name="openedNumber" value="0" />
	...
	<table class="table2">
		...
		<tr>
			...
			<td style="text-align:left; padding:10px 20px 10px 20px;">
				...
				<table style="width:800px;">
					...
					<tr>
						<td style="border:none" colspan="2">
							<span id="sendFrequencyInfo"></span>
						</td>
					</tr>
		...
		<td style="text-align:left; padding:10px 20px 10px 20px;">
		<!--
		<input type="radio" id="openType2" name="openType" onclick="controlParamModel()" value="2" <#if (autoPlan?? && autoPlan.openType?? &&autoPlan.openType==2)|| !autoPlan??>checked</#if> /><label for="openType2">打开应用</label><br />
		<input type="radio" id="openType1" name="openType" onclick="controlParamModel()" value="1" <#if (autoPlan?? && autoPlan.openType?? &&autoPlan.openType==1)>checked</#if>  /><label for="openType1">打开网页</label> 
		...
		<input type="radio" id="openType3" name="openType" onclick="controlParamModel()" value="0" <#if (autoPlan?? && autoPlan.openType?? &&autoPlan.openType==0)>checked</#if> /><label for="openType3">传递参数</label><br />
		-->
		...
		<tr>
		<td colspan="2"><button type="button" onclick="checkForm()">确认</button>&nbsp;&nbsp;<button type="button"  onclick="javascript:window.location.href='/autoPush/list.go'">取消</button></td>
		</tr>
	</table>
</form>
</div>
...
<script type="text/javascript" >
	$(function(){
		...
		<#if customContentMap?? >
		    <#list customContentMap?keys as testKey>  
		    		if('planId' != '${testKey}'){
		    			$('#dd_push_key'+i).val('${testKey}');
						$('#dd_push_value'+i).val('${customContentMap[testKey]}');
						i++;
		    		}
		    </#list>  
		</#if>
		...
		<#if customContentIOSMap?? >
		    <#list customContentIOSMap?keys as testIOSKey>  
		    		if('planId' != '${testIOSKey}'){
		    			$('#dd_push_ios_key'+iosIndex).val('${testIOSKey}');
						$('#dd_push_ios_value'+iosIndex).val('${customContentIOSMap[testIOSKey]}');
						iosIndex++;
		    		}
		    </#list>  
		</#if>
	...
	function initParamsByConditionId(conditionId){
		...
		$.ajax({
			   ...
			   success: function(msg){
				   ...
				   for(var j=0; j<msg.length; j++){
					   $('#planConditionParamsSpan').append('<span>'+msg[j].keyInputName+':</span><input type="text" defType="'+msg[j].keyType+'" name="conditionParam_'+msg[j].pushConditionParamsDefinitionId+'" /><br /><br />');
				   }
			   }
			});
	...
	function initStrategiesByApp(appId){
		
		$("#pushStrategyId option[value!=0]").remove();
		changeCustomContentByStrategyId(false);
	...
	function changeCustomContentByStrategyId(realchange){
	...
	function initConditionsByApp(appId){
	...
	function submitForm(){
		...
		if($('#pushScope1').is(':checked')){
			if(confirm('确定发送给线上全部满足条件的用户吗？如果不确定，请联系您的上级')){
				$('#add').submit(); 
			}
		}else{
			if(confirm('确认提交？')){
...
//		}
		...
		if($('#deviceTypeIos').is(':checked')){
			...
			$.ajax({
				   type: "POST",
				   url: "${rc.contextPath}/manualPush/validateIosLength.go?"+$("#add").serialize(),
				   async: false,
				   cache: false,
				   data: {
					    "timestamp":new Date().getTime()
