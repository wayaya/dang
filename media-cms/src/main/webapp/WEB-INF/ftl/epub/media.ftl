...
div#rMenu ul li{
  ...
  <TABLE border=0 height="100%" align=left>
	<TR>
		...
		<IFRAME ID="testIframe" Name="testIframe" FRAMEBORDER=0  width="100%"  height="1100px;" SRC="/epub/list.go?code=DZS">
		</IFRAME></TD>
	</TR>
</TABLE>
...
<div id="rMenu">
	...
	var demoIframe;
		...
		callback: {
			beforeClick: function(treeId, treeNode) {
				...
				if(treeNode.id ==0){
					demoIframe.attr("src","/epub/list.go");
					zTree.expandNode(treeNode);
					return false;
				}
				if (treeNode.isParent && treeNode.id !=0) {
					demoIframe.attr("src","/epub/list.go?parentId="+treeNode.id);
					zTree.expandNode(treeNode);
					return false;
				} else {
					demoIframe.attr("src","/epub/list.go?parentId="+treeNode.id);
	...
	$.post("/catetory/getCatetory.go?code=DZS", null,
			   function(data){
			     var t = $("#tree");
			     zNodes = data;
