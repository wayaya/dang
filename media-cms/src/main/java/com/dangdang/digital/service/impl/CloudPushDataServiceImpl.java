...
public class CloudPushDataServiceImpl extends BaseServiceImpl<CloudPushData, Long> implements ICloudPushDataService{
	...
	@Override
	public void deleteTodayDataByPlanId(Date date, Long cloudPushPlanId) {
		
		CloudPushData data = new CloudPushData();
		data.setPlanId(cloudPushPlanId);
		data.setCreationDate(DateUtil.getOnlyDay(date));
		this.deleteByParamsObjs(data);
	}
